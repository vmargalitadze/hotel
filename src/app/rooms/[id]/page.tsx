    ...
    <div className="max-w-7xl mx-auto px-4 mt-16 py-10 font-cormorant">
      ...
      <div className="grid md:grid-cols-3 gap-8">
        ...
        <div className=" text-[20px] mt-2">
          <span className="text-[20px] font-bold">ფასი:</span> ₾{room.price}</div>
        {room.discountPrice && <div className="text-green-600 text-[18px] mt-1">  
          <span className="text-[20px] font-bold">₾{room.discountPrice}</span> ₾{room.price}</div>}
        <div className=" text-[20px] mt-2">
            <span className="text-[20px] font-bold">ტევადობა:</span> {room.capacity} სტუმარი</div>
        {room.size && <div className="text-gray-700 text-[18px] mt-2">ზომა: {room.size} მ²</div>}
        <div className=" text-[20px] mt-2">
          <span className="text-[20px] font-bold">შესვლა:</span> {room.checkInTime} | <span className="text-[20px] font-bold">გასვლა:</span> {room.checkOutTime}</div>
